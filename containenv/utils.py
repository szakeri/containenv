...
class CommandContext(object):
    ...
    def _run_command(self, argv):
        ...
        command module *must* have the same name as the command it represents
        as the import system is used to retrieve the command module by the
        command name.

        Command modules are expected to have a self.module_doc string that is
        parseable by docopt and contain a callable object named Command. If the
        Command object has a schema attribute, the arguments passed to the
        command will be validated against the schema before the command is
        called.

        Args:
            argv: The list of command line arguments supplied for a command.
                The first argument is expected to be the name of the command to
                be run.
        ...
        if command_name == 'help':
            if len(argv) == 2:
                ...
                self._help(None)
        logger.info(
            'Running command "%s" with args: %s', command_name, argv[1:])
    ...
    def _get_command_module(self, command):
        ...
        Args:
            command: The name of the CLI command to use in finding a module in
                the commands package.

        Returns:
            A module object for the given CLI command with a valid docopt
            __doc__ and a main() function that takes a list containing the
        ...
        try:
            ...
            raise ValueError(
                '"{}" is not a containenv command. \'containenv help -a\''
                'lists all available subcommands.'.format(command)
            )
    ...
    def _help(self, help_subject_cmd):
        ...
        Args:
            help_subject_cmd: If a help_subject_cmd value is supplied then
                print the help message for the command module if available. If
                the help_subject_cmd is '-a' or '--all', then print the
                standard help message but with a full list of available
                commands.
        ...
        """
        logging.debug(
            'Inside self._help command is {}'.format(help_subject_cmd))
        if not help_subject_cmd:
            ...
            raise ValueError(
                "Unrecognized option '{}'.".format(help_subject_cmd))
        else:
    ...
    def execute(self):
        ...
        If the command is 'help' then print the help message for the
        subcommand; if no subcommand is given, print the standard help message.

        Note:
            This is the entry point method for the containenv command defined
            in setup.py.
        """
